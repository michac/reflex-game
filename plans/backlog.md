...
### R9 · Pick a style winner — `idea`
The game ships the **wireframe** skin (neutral gray). Author style variants the Star Slingers way: copy `mockups/style-wireframe.html`, swap the tokens `<link>`, define a `tokens-<style>.css` (e.g. neon, vector). Compare on the phone; the winner's tokens become `src/tokens.ts`.
